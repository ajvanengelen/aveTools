...
def allpowersFitsFile(beforeroot, afterroot, window = None, beamFile = None, flipQ = False, TOnly = True, method = 'pure', useI = True, binFile = None):
    ...
    if TOnly == False:
        ...
        output = allpowers(T_map, Q_map, U_map, window = window, beamFile = beamFile, flipQ = flipQ, method = method, binFile = binFile)
    else:
        T_map = liteMap.liteMapFromFits(beforeroot + ('I' if useI else 'T') + afterroot)
...
def mpiSendReceiveList(myList, comm, iMin, iMax, delta, iStart = 0):
    #modify myList in place.  Assume that rank = 0 zero is the queen bee.
    ...
    if rank > 0:
        comm.send(myList[iMin:iMax], dest = 0)
        print 'rank %d of %d: sending data, length' %(rank, size), len(myList[iMin:iMax])
    else:
        for i in range(1, size):
            ...
            myList[iStart + i * delta : iStart + i * delta + len(inData)] = inData
            print 'rank %i of %i: received data from rank %i ' % (rank, size, i)
    return 
